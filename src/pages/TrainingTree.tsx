...
const checkmatesData = {
    "Mates": [
        { name: "General Checkmates", items: [
            { id: "mateInOne", name: "Mate in 1", description: "A checkmate delivered in a single move." },
            { id: "mateInTwo", name: "Mate in 2", description: "A checkmate requiring two moves to complete." },
            { id: "mateInThree", name: "Mate in 3", description: "A checkmate requiring three moves to complete." },
            { id: "mateInFour", name: "Mate in 4", description: "A checkmate requiring four moves to complete." },
            { id: "mateInFive", name: "Mate in 5", description: "A checkmate requiring five moves to complete." },
            { id: "mate", name: "Mate in ?", description: "A checkmate puzzle with an unknown number of moves." }
        ]},
        { name: "Named Checkmates", items: [
            { id: "anastasia", name: "Anastasia’s Mate", description: "A checkmate involving a rook and knight against the king trapped on the h-file." },
            { id: "backrank", name: "Back-rank Mate", description: "A checkmate delivered by a rook or queen on the back rank when the king is blocked by its own pawns." },
            { id: "doublebishop", name: "Double Bishop Mate", description: "A checkmate delivered by two bishops, typically forcing the king to the edge of the board." },
            { id: "hook", name: "Hook Mate", description: "A checkmate pattern using a rook, knight, and pawn to trap the king." },
            { id: "vukovic", name: "Vuković's Mate", description: "A checkmate pattern often involving a rook and pawn against a king on the edge of the board." },
            { id: "arabian", name: "Arabian Mate", description: "A checkmate pattern involving a knight and a rook." },
            { id: "boden", name: "Boden’s Mate", description: "A checkmate pattern where the king is checkmated by two bishops on intersecting diagonals." },
            { id: "dovetail", name: "Dovetail Mate", description: "A checkmate pattern resembling a dovetail joint, often with a queen and another piece." },
            { id: "killbox", name: "Kill Box Mate", description: "A checkmate where the king is trapped in a small 'box' by attacking pieces." },
            { id: "smothered", name: "Smothered Mate", description: "A checkmate delivered by a knight when the king is completely surrounded and blocked by its own pieces." }
        ]}
    ],
    "Phases": [
        { name: "Opening", items: [
            { id: "openingPrinciples", name: "Learn key principles", description: "Understand fundamental strategies for the initial phase of the game." },
            { id: "commonOpenings", name: "Study common openings", description: "Familiarize yourself with popular opening sequences." }
        ]},
        { name: "Middlegame", items: [
        ...
        { name: "Endgame", items: [
            { id: "queenRookEndgame", name: "Queen & Rook", description: "Master endgames involving a queen and a rook." },
            { id: "rookEndgame", name: "Rook", description: "Study common rook endgame principles and techniques." },
            { id: "queenEndgame", name: "Queen", description: "Learn strategies for queen endgames." },
            { id: "knightEndgame", name: "Knight", description: "Understand knight endgame principles and common patterns." },
            { id: "bishopEndgame", name: "Bishop", description: "Explore bishop endgame strategies and maneuvers." }
        ]}
    ],
    "Lengths": [
        { name: "Puzzle Duration", items: [
            { id: "oneMove", name: "One Move", description: "Puzzles that can be solved in a single move." },
            { id: "shortMoves", name: "Short (2 moves)", description: "Puzzles requiring exactly two moves to solve." },
            { id: "longMoves", name: "Long (3 moves)", description: "Puzzles requiring exactly three moves to solve." },
            { id: "veryLongMoves", name: "Very Long (4+ moves)", description: "Challenging puzzles requiring four or more moves." }
        ]}
    ],
};

// Data for the right tree (Tactics & Positional)
const tacticsData = {
    "Tactics": [
        { name: "Basic Tactics", items: [
        ...
        { name: "Advanced Tactics", items: [
            ...
            { id: "captureDefender", name: "Capture the Defender", description: "A tactic where you capture a piece that is defending another piece or a critical square." },
            { id: "doubleCheck", name: "Double Check", description: "A powerful check where two pieces simultaneously attack the king." }
        ]}
    ],
    "Advanced": [
        { name: "Positional Concepts", items: [
...
const TreeBranch = ({ category, subCategories, isOpen, onToggle }) => (
    <div className="relative pl-8">
        ...
        <div className="relative pl-8">
            <h3
                onClick={onToggle}
                className="text-xl font-bold text-slate-800 cursor-pointer hover:text-blue-600 transition-colors select-none"
            >
                {category}
            </h3>
            {isOpen && (
                <div className="mt-4 space-y-4">
                    {subCategories.map(sub => (
                        <div key={sub.name} className="p-4 bg-slate-100/70 rounded-lg border border-slate-200/80">
...
export default function App() {
    return (
        <div className="min-h-screen w-full flex flex-col items-center pt-8 md:pt-16">
            ...
            <p className="text-center text-slate-500 mb-10">Click on a training method to see its description.</p>
